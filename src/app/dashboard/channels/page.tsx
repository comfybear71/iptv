...
export default function BrowseChannelsPage() {
  ...
  // Load categories once we have credentials
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // categories are non-critical — channel list still works without them
      }
    })();
  }, [hasCreds]);
  ...
  // Load channels — optionally filtered by selected category
  const loadStreams = useCallback(
    async (opts: { page: number; search: string; category: string | null }) => {
  ...
  useEffect(() => {
    if (!hasCreds) return;
    loadStreams({ page, search, category: selectedCategory });
  }, [hasCreds, page, search, selectedCategory, loadStreams]);
        ...
        <>
          ...
          <div className="mt-6 flex flex-col lg:flex-row gap-4">
            ...
            <div className="lg:hidden w-full">
              ...
              {catOpen && (
                <div className="mt-1 max-h-64 overflow-y-auto rounded-xl border border-slate-800 bg-slate-900">
                  <button
                    onClick={() => { setSelectedCategory(null); setPage(1); setCatOpen(false); }}
                  ...
                  {categories.map((cat) => (
                    <button
                      key={cat.category_id}
                      onClick={() => { setSelectedCategory(cat.category_id); setPage(1); setCatOpen(false); }}
                      className={`flex w-full items-center justify-between px-4 py-2.5 text-left text-sm ${
                        selectedCategory === cat.category_id
                          ? "bg-purple-600 text-white font-semibold"
                          : "text-slate-300 hover:bg-slate-800"
            ...
            {/* Channel list */}
            <section className="min-w-0 flex-1 overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 p-5">
              <div className="flex items-center gap-3">
                <input
                  type="text"
                  value={search}
                  ...
                  placeholder={
                    selectedCategory
                      ...
                      : "Search channel name (e.g. CNN, Fox, Seven)"
                  }
